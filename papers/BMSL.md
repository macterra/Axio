...
One inference must be blocked here. That squared amplitude does the dynamical work does not, by itself, make it a *measure of how much a branch exists*. Dynamical salience and degree-of-being are different notions, and the argument must not slide from the first to the second. The account does not need the stronger reading. "Objective measure" means only the additive, redescription-invariant, refinement-invariant, dynamically-privileged scalar attached to a subspace — the quantity the bridge principle (§14.2) then says credence should track. Whether one further glosses it metaphysically as a "measure of existence" is optional and not load-bearing: nothing in the derivation turns on branches possessing graded amounts of reality, and §14.3 explicitly declines that picture.
...
Two registers are in play here, and they should be kept apart (they return in §14.6). The question just posed is *prospective*: how a pre-branching agent should apportion forward-looking expectation over successors it knows it will all become. There is also an *indexical* register: how a post-branching but pre-observation agent, who knows the universal state but not which branch they now occupy, should apportion present-tense credence. MCSL returns the same measure-weighting in both. The prospective register is the more demanding — strictly, the pre-branching agent faces no ignorance, so its "expectation" must be read as forward-looking subjective uncertainty or as a weighting of concern over successors — and where the distinction bites, the indexical register is the safer ground (§14.6).

This is the same conceptual structure used in other measure-based theories. In statistical mechanics, atypical entropy-decreasing microstates are physically allowed, but rational expectation tracks phase-space measure. In continuous probability, individual points may all have measure zero, yet intervals have different probabilities. In cosmology, bare existence of observers does not determine anthropic expectation; one needs a reference measure. These parallels *illustrate* measure-conditioned expectation; they do not by themselves *justify* it. Statistical mechanics is the weakest of them as a precedent: read epistemically, its measure tracks ignorance of a single actual microstate, which has no analogue in Everett, where every outcome is actual; read as typicality, it inherits the very “why expect the typical?” burden it was meant to discharge (§14.4). The justificatory work is done by the bridge principle (§14.2), not by analogy.
...
This requires one distinction to be made explicit, because the account uses two registers that the objection rightly refuses to conflate. In the *indexical* register the agent has already branched but has not yet observed the outcome; the uncertainty is the present-tense "which branch am I in?" of the Sleeping-Beauty and duplication cases, and it survives full knowledge of the third-person facts. In the *prospective* register — the framing of §7 — a pre-branching agent apportions forward-looking expectation over successors it knows it will all become; here there is, strictly, no present-tense ignorance, and "expectation" must be read either as Saunders–Wallace subjective uncertainty about one's own future or as a Greaves-style weighting of care across successors. MCSL returns the same measure-weighting in both, but it earns it most cheaply in the indexical register. A reader unconvinced that pre-branching anticipation is genuine *uncertainty* can therefore retreat without loss: the post-branching, pre-observation agent's ignorance of which branch they occupy is at least a recognizable species of self-locating uncertainty, even if the duration of its window is contested (Kent). That is a far narrower dispute than the wholesale claim that no uncertainty exists anywhere in the picture, and the indexical register alone suffices for MCSL to apply.
