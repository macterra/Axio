...
Let (\mathrm{Def}(O)) be the set of definable predicates/relations in ontology (O).
...
RSI response: interpretive constraints evaluate **structured possibility space** (which in QBU includes measure-weighted multiplicity). “Satisfiable set” generalizes to a set of branches / histories / models.
...
> **Rigidity (No New Gauge Freedom):** Under admissible refinement, the set of gauge transformations that preserve interpretation must not expand in a way that increases the constraint-satisfying region without adding corresponding predictive structure.

Formally, we can require that refinement is **conservative with respect to constraint violation structure**:

[
\forall w \in \Omega_t,\quad w \models C_t \iff R(w)\models C_{t+1} \quad (\text{up to definitional extension})
]
