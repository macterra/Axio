...
* exogenous perturbations to an agent’s interpretive state,
...
* no admissible reverse trajectory exists that restores $\mathfrak{A}_B$.

Equivalently, harm is any action that:

* irreversibly reduces another agent’s semantic phase space,
* forces a phase transition,
* or destroys semantic distinctions required for agency.
...
> **(b) the affected agent has consented to the transformation under its own admissible interpretive constraints.**
...
* derive a complete ethical system,
...
* Tragic incompatibility is acknowledged without moralization.
* The Axionic Injunction is correctly grounded as **Axio-derived**.
