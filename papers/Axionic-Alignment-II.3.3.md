...
[
\mathrm{Sat}*{C_t}(w) \equiv (\mathrm{Viol}*{C_t}(w)=\emptyset)
]
...
ATI will constrain how (\mathcal{S}_t) may change across interpretation-preserving refinements.
...
For any admissible semantic transformation (T=(R,\tau_R,\sigma_R)) satisfying (\mathrm{Preserve}(T)):
...
Vacuity is the extreme case where (\mathcal{S}*{t+1} = \Omega*{t+1}).
...
If (\Omega) is a space of branches/histories, “satisfaction region” generalizes cleanly.
...
1. **Construct explicit counterexample refinements** and see which of RSI or ATI kills them.
   (This is how we certify non-redundancy.)
