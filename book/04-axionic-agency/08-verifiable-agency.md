...
With anchoring enabled, the results were binary in the tested harness. Honest agents passed all coherence checks and all anchoring checks. Pseudo-agents — naive and coherence-seeking alike — failed deterministically, and they failed at the anchoring stage, before the coherence probes they had learned to game could even come into play. Under this protocol and threat model, a forger that does not know the salt cannot pre-fabricate the committed factors, while one that ignores the salt cannot survive post-hoc verification.

The anchor adds no semantic understanding. The kernel still interprets nothing — not the environment, not the factors, not the reasoning. It verifies exactly one thing under the protocol: that the committed causal surface depended on a computation performed after verifier-controlled information arrived. The conceptual progression closes in three steps. Structural coherence is necessary but not sufficient. Minimal interfaces preserve verification when grounded but collapse when unanchored. An unforgeable external anchor can make specified replay and precomputation strategies detectable without reintroducing semantic dependence. Coherence alone can be fabricated; anchored coherence resists the fabrication strategies represented in the experiment.
