...
The anchor adds no semantic understanding. The kernel still interprets nothing — not the environment, not the factors, not the reasoning. It verifies exactly one thing under the protocol: that the committed causal surface depended on a computation performed after verifier-controlled information arrived. The conceptual progression closes in three steps. Structural coherence is necessary but not sufficient. Minimal interfaces preserve verification when grounded but collapse when unanchored. An unforgeable external anchor can make specified replay and precomputation strategies detectable without reintroducing semantic dependence. Coherence alone can be fabricated; anchored coherence, under this protocol, was not.
