...
And the binding is local. This volume's central device, [agent-binding](02-agent-binding.md), holds that a moral claim gets its truth conditions from the situation of a specified agent; the QBU adds a Vantage index to the binding. When alternative promise records have nonzero amplitude, a single global kept-or-broken verdict is useless — what binds is the pattern at the agent's Vantage. Obligations are Vantage-indexed, not universalized across the multiverse: they hold for the agent at this record, given its causal history. This is Conditionalism doing what it always does — moral truths conditional on Vantage, Measure, and persistent pattern, not floating free over the totality.
