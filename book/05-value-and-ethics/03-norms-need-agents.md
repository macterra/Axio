...
The target is world-immanent normativity, not ordinary epistemic realism. One can hold that a claim's accuracy is constrained by a mind-independent target while denying that the target commands anyone to investigate or update. Moral requirements and epistemic obligations are prescriptions; correspondence is a relation between representation and target. There is no cosmic conscience or teleological tribunal. Constraint is real, and it is not obligation. Resistance is real, and it is not reproach.

## Norms Need Agents

Norms do not exist in the wild. They exist in and through rule-following agents and practices; remove those and no one is obligated, permitted, praised, or reproached. Representations can still match or fail to match their targets, because those targets are not created by the modeler. What disappears is the practice of evaluating and correcting them. Causality governs atoms; normativity governs agents.
...
This is the value-side counterpart of what Volume 2 established about truth: [interpreted claims require conditions](../02-conditionalism/02-all-truth-is-conditional.md). Normative claims additionally require a standard. The standard may be an individual's commitment, a shared practice, or a public framework, but it is not supplied by causal description alone. [Agent-binding](02-agent-binding.md) binds that normative input to the agent who holds it, which turns the claim into an empirical statement about that agent — the reverse of deriving an ought from a description, and the one direction that is safe. It does not thereby reduce moral truth to whatever a vantage prefers: a bound value is a fact about an agent, open to evidence and error, and the conditions under which it applies are fixed by condition-binding, not by taste.
...
- **Descriptive (alethic):** what is physically possible — what the laws of nature allow to occur.
- **Agential (ability):** what an agent can in fact do, given its causal and cognitive constraints.
- **Deontic (normative):** what is permitted, required, or forbidden under some standard.
...
**A cold universe, therefore no right and wrong.** False. Moral systems arise among agents embedded in networks of consequence, cooperation, conflict, and reciprocity. Right and wrong are verdicts under normative standards, not divine decrees or invariants generated by coherence alone. The slide here runs from the absence of *cosmic* evaluation to the absence of *all* evaluation, as if the only place a norm could live were the sky. But agents and practices can hold and apply norms, and the cold universe is full of them.
...
There is, in fact, causal accounting. Choices propagate through social systems, feedback loops, and memory. Some destructive strategies rebound on their authors; others persist, profit, or remain insulated for long periods. Coherent agents can preserve or destroy other agents, and incoherent arrangements can survive with external support. The universe does not guarantee moral bookkeeping. It supplies consequences whose moral significance depends on the standard used to assess them.

This is where the two halves of the chapter meet. Against Smith as read here: the world constrains but never reproaches, so epistemic obligations belong to agents and practices even while accuracy remains answerable to the target. Against the nihilist: the world's silence on morals abolishes nothing, because moral normativity belonged to agents and their frameworks all along. The world supplies consequences; a normative premise explains why particular consequences count as reasons, wrongs, or responsibilities. The absence of divine command does not liberate us from consequence or erase the standards we avow. It makes us responsible for stating and defending them.
