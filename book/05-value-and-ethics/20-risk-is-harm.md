...
On the Quantum Branching Universe (QBU) interpretation introduced in [Measure and Credence](../02-conditionalism/11-measure-and-credence.md), branch weight supplies **Measure**, while an agent's uncertainty remains Credence. Even within that interpretation, “twenty percent chance of death” should not be redescribed as 0.20 literal deaths without specifying the event, branch decomposition, and relation between decision weights and persons. The useful point is conditional: an intervention can shift weight toward descendant histories in which an agent's capacity is degraded.
