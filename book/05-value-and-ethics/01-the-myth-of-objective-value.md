...
But morality is made of value judgments, and all value is subjective. The conclusion follows immediately: morality cannot be objective, because it has no raw material to be objective *with*. No valuer, no value. No value, no moral claim.
...
Nor does subjectivism mean that moral claims are beyond criticism, that anything goes, or that the slaveholder's values are as good as yours. Getting from "all value is subjective" to a framework in which moral claims are explicit, evaluable, and genuinely arguable requires one further device — binding each claim to the agent whose values give it content — and that is the work of [agent-binding](02-agent-binding.md).
