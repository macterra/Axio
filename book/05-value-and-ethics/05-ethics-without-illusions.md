...
That is not a diminished consequentialism. It is the only kind that was ever practiced. No utilitarian has ever computed the universal sum; every one of them has weighed outcomes from where they stood, using the values they had, while calling the result objective. Dropping the label is not a loss of rigor. It is the beginning of it.
...
"You are the source of your values" answers the question this chapter asked — what survives the death of objective value — but it immediately raises a harder one. If every code is chosen, are all choices of code on a par? Your values are yours; but you exercise them in a world full of other agents exercising theirs, and different codes have different consequences for coexistence, trust, domination, and survival. Those consequences do not depend on wishing them away. Calling one pattern permissible or better still requires a normative premise.

This chapter is the first pass over that ground — the traditions rebuilt on subjective value, ethics as chosen framework. The second pass, in [The Ethics of Viability](22-the-ethics-of-viability.md), joins an explicit commitment to reciprocal authorship with evidence about which codes sustain or attack it under stated conditions. The freedom established here is real and nothing in what follows retracts it. What follows maps its consequences and the framework's chosen limits.
