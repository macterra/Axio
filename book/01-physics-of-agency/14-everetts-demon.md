...
Quantum mechanics establishes, experimentally and independent of interpretation, that microscopic events do not have single predetermined outcomes. Whether a particular ion channel opens, whether a synaptic vesicle releases its neurotransmitters, how a molecular binding resolves — these are not deterministic facts awaiting a sharper microscope; the theory assigns amplitudes to alternatives, not a unique result. And in the [Quantum Branching Universe (QBU)](08-the-quantum-branching-universe.md), those alternatives do not stay merely possible: as decoherence renders them dynamically independent, they settle into distinct branches — approximate, emergent histories at the relevant grain rather than a bookkeeper's exact per-event forks, but real, and irreversibly diverging.
...
It would, in a system that damped it out. Brains, this chapter conjectures, may do the opposite. Neural signaling runs on components that operate at scales where quantum fluctuations can matter: probabilistic neurotransmitter release, single ion channels gating open or closed, molecular bindings resolving one way or the other. A neuron sitting near its firing threshold is a plausible detector of exactly this noise — a sub-threshold fluctuation could decide whether it spikes now, a few milliseconds later, or not at all. Feed that spike into a recurrent network that discriminates, decides, and acts on fine differences, and the microscopic variation need not wash out; it can cascade. Whether biological brains actually exploit this route rather than merely tolerate it is an open empirical question — but nothing in their architecture obviously forbids it.
