...
The physics of agency finds a suggestive comparison in Karl Friston's work.
...
I arrived at [the three laws of agency](04-the-three-laws-of-agency.md) from a different direction entirely: from thermodynamics, from Landauer's principle, from asking what it physically costs to steer a probability distribution over outcomes — with the distributional shift quantified in [kybits](03-the-kybit.md). Nothing in that argument begins from brains, prediction, or variational free energy. Set the two frameworks side by side and a partial structural correspondence appears, law by law. It is evidence of a shared problem structure, not yet a derivation or a conversion between their quantities.

## The First Law: Control Is Work

The first law says that exercising intentional control requires a resource-consuming physical implementation. Only under an equilibrium-reference realization does the kybit divergence supply the proposed minimum-work bound.
...
Friston's framework does not promise frictionless control. Variational inference is performed by a finite model through limited observations and actions; real agents face environmental fluctuation, sensory noise, model error, and finite precision. A particular free-energy value need not translate directly into control capacity, and the formal objective alone does not prove the third law. It does exhibit the same bounded architecture: improving a model or action policy does not grant complete access, perfect prediction, or costless actuation. Absolute predictive precision and frictionless control remain unavailable to an embedded finite agent.
...
The convergence is evidence, not proof. It could reflect a shared problem, a productive analogy, or a shared hidden error. Active inference offers candidate mechanisms for how some agents build, run, and correct predictive models. The three laws insist that any implementation face physical bookkeeping and boundedness. What emerges from the comparison is a research program: test when predictive control, kybit divergence, variational free energy, and work can be related within one causal model. Two roads, cut through different country, reaching a common pass — without yet proving that they end at the same place.
