...
- **A defined baseline.** The claimed difference is a comparison, and the comparison class is a counterfactual: the Measure distribution over the Branchcone forward of the agent's Vantage *had the agent's predictive machinery been inert* — the baseline distribution $P_{\text{initial}}$, restated as a fact about event weights. That counterfactual must be constructed with the branch-based semantics of [Causality and Counterfactuals](10-causality-and-counterfactuals.md) and shown to be well-defined in a branching ontology, not smuggled in from single-world intuitions.
