...
An idea arrives. You did not assemble it deliberately, step by step; it surfaced, whole enough to notice, and only then did you turn conscious attention on it to see whether it was any good. This is the ordinary phenomenology of a new thought, and it is exactly what makes creativity feel mysterious — a faculty that hands you finished candidates from nowhere, as if by a spark. I want to dissolve the mystery. Creativity is not a spark. It is *virtual* evolution: variation subjected to selection, the same two-move logic that drove the evolution of every organism — but run inside a single head rather than across a breeding population, and so without inheritance, reproduction, or differential survival in the literal biological sense. What carries over is the logic, not the machinery: randomness generated, then filtered, freed from the slowness of genes and the tyranny of a fixed environment. The spark is what the process looks like from the inside, after the filtering has already done its work.
...
If variation is the raw material, what supplies it? Most of it is ordinary neural stochasticity — noisy firing, spontaneous recombination, associative recall dragging distant material into contact. A nervous system is a prolific generator of candidate perturbations without reaching for exotic physics. And *if* the [Everett's Demon](14-everetts-demon.md) argument holds — that neural processing can be sensitive to quantum divergence, amplifying it into divergent macroscopic outcomes — then some of that variation traces deeper still, to a well of candidates with no floor. But nothing in the account below depends on the quantum claim; classical noise or quantum, the brain faces the same task and solves it the same way: it turns a well of noise into a stream of ideas.

It does this by being a filter, not a funnel. A prolific source of variation does not mean every brain generates every possible idea — a misreading that would make the theory absurd. Each individual's neural architecture is a unique selective amplifier: raw perturbations propagate through *this* wiring, and only particular ones get amplified into fully formed, macroscopic cognitive differences. Your neural configuration fixes which subset of idea-space you can reach at all. So the picture is layered from the start: stochastic perturbation is the microscopic variation, and neural architecture is already the first selection, deciding which variations are even allowed to become thoughts. Two people facing the same problem explore different regions of possibility not because the underlying noise differs but because their filters do.
...
- **Unconscious variation.** Networks spontaneously throw up candidates — stochastic firings, recombinations of stored patterns, associative recall pulling distant material into contact.
- **Unconscious selection.** The first cull is implicit and ruthless, governed by the predictive machinery that any agent runs to reduce its uncertainty about what comes next — the predictive-modeling criterion that [separates agents from mere mechanisms](05-minimal-and-maximal-agents.md), here turned inward on the mind's own candidates. Prediction-error and free-energy minimization discard the overwhelming majority before they are ever felt.
- **Preconscious processing.** Survivors are weighed by emotional resonance, intuition, and subconscious pattern recognition, which amplify the ones carrying implicit value.
- **Conscious selection.** Only a handful reach awareness, where explicit evaluation, reflection, and reasoning perform the final prune. What you experience as "having an idea" is contact with a late-stage survivor, not with the generator.
