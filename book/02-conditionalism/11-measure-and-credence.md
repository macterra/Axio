...
I call this picture the **Quantum Branching Universe (QBU)**. It represents the universal quantum state through emergent, effectively autonomous record histories at a declared coarse-graining. “Branch,” “timeline,” and “world” are useful shorthand for those histories, not uniquely countable fundamental objects, and our record has no privileged status within the model. A static family of block-like histories is one visualization; the formal construction and its limits are in [The Quantum Branching Universe](../01-physics-of-agency/08-the-quantum-branching-universe.md). For everything in *this* volume, four commitments suffice:

1. **Branching, not collapse.** Unitary evolution plus decoherence yields effectively autonomous records; no fundamental collapse selects one.
2. **Multiple records are real.** Alternatives with nonzero amplitude can be represented in the state; this does not imply that every imaginable outcome occurs.
...
**Credence** is subjective probability: an agent's rational degree of confidence in a proposition, given the information the agent has. In branching terms, a central case is self-locating uncertainty about which record the agent occupies or will observe, while allowing that more than one successor may exist. A Credence is a fact about an agent's epistemic state, and so it legitimately varies from agent to agent. Your Credence that the coin lands heads might be 0.5; mine, having watched the same coin come up heads eleven times running, might reasonably sit elsewhere. The world has not changed between us — our information has. Credence is the raw material of belief: as I argued in [What Beliefs Are](08-what-beliefs-are.md), to believe a proposition just is to assign it Credence high enough to act on.
...
The conflation does its worst damage in the philosophy of science. When I assign a Credence of 0.9 to a scientific theory, I am not asserting that the theory is 90% true, or true in 90% of branches, or possessed of some intrinsic probabilistic essence. Theories are not the kind of thing that has a Measure. The 0.9 quantifies *my* rational uncertainty about whether the theory correctly describes reality — a fact about me, not about the theory. Critics of Bayesian epistemology, David Deutsch and Brett Hall foremost among them,[^hall] have effectively skewered the confused version of this claim, and their deeper challenge — that explanatory knowledge leaves no legitimate role for credence at all — deserves a full answer, which I give it [in defense of Bayes](12-in-defense-of-bayes.md). For now the point is narrower: the position they rightly attack, probability as a property of theories themselves, is exactly what the Measure/Credence distinction forbids.

Note also what the distinction does *not* say: neither kind of probability is unconditional. A Measure is always the Born weight of a specified event sector given a state, conditioning record, and decomposition; a Credence is always confidence given evidence and background assumptions. Both are conditional through and through, as [all truth is conditional](02-all-truth-is-conditional.md) — the distinction between them is not conditional versus absolute, but physical model versus epistemic state.
...
Take the coin flip, with an observation layered on top. The hypothesis $H$: the coin landed heads. The evidence $E$: I glimpse the coin in bad light and it looks like heads. Both pick out record sectors, and the accepted physical model fixes their weights:

- $\text{Measure}(E \mid H)$ — within the heads sector, the relative weight of records in which I see heads. Say my glimpse is 90% reliable: 0.9.
- $\text{Measure}(E)$ — the overall weight of records in which I see heads, however the coin landed. For a fair coin: $0.9 \times 0.5 + 0.1 \times 0.5 = 0.5$.
...
[^hall]: Brett Hall, “Bayesian Epistemology,” bretthall.org, https://www.bretthall.org/bayesian-epistemology.html.
