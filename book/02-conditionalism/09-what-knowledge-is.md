...
**Tacit knowledge** — the bicycle, the violin, the seasoned diagnostician's hunch — is embodied, implicit, and resistant to explicit formulation. It passed the stress test above on the same terms as everything else. The definition never required knowledge to be statable, only to be encoded and reliable, so tacit knowledge belongs in the full taxonomy rather than a footnote.
