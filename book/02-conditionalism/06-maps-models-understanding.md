...
sources:
  ...
  - 167116201.the-map-is-not-the-territory
---
...
This is Conditionalism's home territory. Model mediation is the normal form of empirical knowledge, and models encode many conditions under which claims are assessed. Empirical truth is not merely coherence inside a model: observation, intervention, prediction, and resistance from the target constrain which coherent models survive. Understanding, on this account, is the ability to construct, compare, use, and revise models while tracking their domains and omissions.
...
Some evaluative work can be represented in Bayesian terms, whether or not a scientist writes down a prior; other valid procedures emphasize likelihood, error control, robustness, or severe testing. General relativity earned support by explaining observations and surviving tests that exposed Newtonian limits. A reported parameter estimate is conditional on a cosmological model, data, calibration, and an inferential procedure. Bayesian posteriors answer questions about parameters or models given priors and likelihoods. Frequentist intervals answer questions about the long-run calibration of procedures, which researchers and regulators often need. Neither framework earns universal jurisdiction by caricaturing the other's question.

Probability, in this role, is one quantitative language for uncertainty under stated conditions; probability is not itself a truth value. It can represent an agent's Credence across models. Under the optional Quantum Branching Universe (QBU) module, a distinct objective weight is proposed too: Measure over specified quantum alternatives. Keeping those roles separate is central to Part V.

Model dependence reaches into statistical mechanics. Macrostates such as temperature and pressure coarse-grain vast numbers of microstates, and entropy depends on the ensemble and variables used. That does not make the second law merely a law about our maps: its thermodynamic content constrains physical processes under specified boundary conditions. Quantum interpretations provide another test case. Standard formulations may share operational predictions while assigning different ontological roles to the state. Choosing among them involves explanatory virtues as well as empirical adequacy; [Probability Without Collapse](14-probability-without-collapse.md) states this book's conditional Everettian choice.
...
**Pretending completeness.** A purpose-relative map must omit. A formally complete description may be possible for a bounded target under a chosen language, but reproducing every irrelevant detail would defeat many purposes of modeling. The dangerous cartographer is not the one whose map has blank regions but the one who denies them.
