...
As systems scale, these costs can outrun the benefits of coordination. Centralization often buys efficiency by discarding interpretive resolution — by compressing away the context that made evaluation possible. The loss is cumulative and hard to reverse because restoring institutional authorship requires reconstructing context already thrown away. You cannot decompress a file whose details were deleted in the compression. Leviathan is therefore an attractor under large-scale coordination, not proof that every large institution has already collapsed into it. Transparency, modularity, narrow jurisdiction, traceable delegation, and real exit can move the threshold; once those safeguards are absorbed into procedure rather than restoring authorship, exit and rebuilding at viable scale may be the only remedy left.
