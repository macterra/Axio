...
sources:
  ...
  - 167356408.justice-without-juries
---
...
So every assertion of certain guilt is in fact a Credence — a subjective probability — disguised as a fact. What we would like to know is the [Measure](../01-physics-of-agency/09-measure-vantage-branchcone.md), the objective weight of the worlds in which the accusation is true; what we actually have is a [Credence conditioned on partial, polluted evidence](../02-conditionalism/11-measure-and-credence.md), and the two must never be confused. Every claim of guilt is [conditional](../02-conditionalism/02-all-truth-is-conditional.md) on background assumptions — on the reliability of a witness, the calibration of an instrument, the honesty of an informant, the interpretation of an ambiguous act. A trial is the societal machinery designed to drag those conditions into the light and test them one by one.
...
So: the indictment. The jury system fails on five counts, and each count is an epistemic failure, not a procedural one.
...
**The accountability gap.** A juror who reasons carelessly, ignores instructions, or votes from prejudice faces no consequence of any kind. The verdict is unexplained, unappealable on its reasoning (there is no reasoning on the record to appeal), and owned by no one. Every other actor in the courtroom can be reviewed, sanctioned, or overturned; the twelve people who actually decide are answerable to nothing.
...
**Professional lay-judge tribunals.** Germany and Denmark already run mixed panels of professional judges and trained lay judges — lay participants who receive explicit instruction in logic, bias awareness, and evidentiary standards before they ever hear a case. The democratic element survives; the epistemic floor rises. This is the least radical reform, and it is already working elsewhere.
...
**AI decision support.** Systems trained on precedent, empirical base rates, and logical inference can flag inconsistencies in testimony, surface biases in the record, and map exactly where the evidentiary chain is weakest. The design constraint is absolute: the machine assists; it never decides. The moment the tool becomes the judge, accountability evaporates into a model no one can cross-examine — a specific instance of the general danger of [confusing an instrument's outputs with judgment](../03-minds-and-machines/18-tool-bias.md). Kept in its place, it does for the court what rules of evidence do: it makes certain classes of error harder to commit invisibly.

**Prediction markets.** Markets aggregating expert credences about factual questions in a case would make the epistemic state of the evidence transparent — where informed observers converge, where they split, and how confidently. [Markets of this kind](../06-markets-and-money/17-mechanisms-for-honest-values.md) pay participants for being right, which is more accountability than any juror has ever faced. They should inform deliberation rather than replace it, but the underlying principle — institutions that elicit honest evaluations instead of performed ones — is the same one this volume builds to in [Axiocracy](35-axiocracy.md), applied here to the discovery of truth rather than value.
...
My actual recommendation combines the strands: professional epistemic judges at the core, AI analysis as decision support, prediction markets as transparent input on contested factual questions. Human beings remain accountable for every verdict; the technology and the markets exist to make their errors visible before they are made, not after.

## One Argument, Two Levels

The two halves of this chapter are sometimes read as opposites — the first venerating trial process, the second attacking it. They are one argument. The presumption of innocence, the burden of proof, and adversarial scrutiny are the *specification*: no violence without the strongest available filter against error. The jury is an *implementation* of that specification, and implementations are fungible. Judged by the only standard that matters — how well it filters errors — the jury fails on five counts, and better designs exist. Conversely, every proposed reform must be judged by the same specification it would serve: a system that raised accuracy but shifted the burden of proof onto the accused would be a regression, whatever its technology, because it would have optimized the machinery by abandoning the function.
