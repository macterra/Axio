...
From the agency perspective this is exactly what the theory predicts. Harm is [a contraction of reachable futures](../05-value-and-ethics/11-what-counts-as-harm.md) — enforced impossibility, not difficulty. What matters for flourishing is not where one sits on a ladder at time *t*, but whether future ladders remain reachable. A society with high variance and open paths preserves agency. A society with frozen positions destroys it. Treating those cases as morally equivalent because they share a Gini coefficient is a category error. It also explains why redistribution schemes that cosmetically flatten outcomes while leaving barriers intact so often fail to improve lived experience: they optimize a surface metric while quietly shrinking the set of futures a person can realistically pursue.
