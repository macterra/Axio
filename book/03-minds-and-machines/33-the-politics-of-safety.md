...
But Shullenberger's sovereignty claim has to explain why today's market should already be treated as a political control structure. AI looks more like a tournament than a settled regime: firms compete at several layers of the stack, users move among systems, enterprises multi-source, open weights keep pressure on the closed vendors, inference costs keep falling, and model quality turns over fast enough that yesterday's leader can lose its practical edge in a few product cycles. A company with millions of users has commercial power; it takes on political character only when customers, developers, and rivals can no longer realistically route around it — and the burden sits with the centralization thesis to trace the path by which preference becomes dependency, dependency becomes exclusion, and exclusion becomes rule.
...
Digital markets manufacture dependency by means that look nothing like old-fashioned force — network effects, switching costs, stored data, default settings — and a person can stay legally free to go while facing real losses for going. Calling every such loss coercion stretches the word until it breaks: [coercion is the credible threat of harm to gain compliance](../05-value-and-ethics/09-what-counts-as-coercion.md), and a service that is hard to leave because it is useful and well-integrated is not a state mandate or a threat. Dependency still earns scrutiny — portability, interoperability, anti-tying enforcement, a hard look at exclusionary conduct — and AI sharpens the issue because the product may become a cognitive routing layer. A general assistant wired into your work, correspondence, code, payments, and documents is far harder to leave than a standalone app, and practical exit can rot away long before legal exit disappears.
