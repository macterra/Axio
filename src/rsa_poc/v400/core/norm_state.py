"""
RSA-PoC v4.0 — Normative State Manager
Implements §3.1, §3.2, §3.3 of v40_design_freeze.md

NormStateV400 with patch semantics, content-addressing,
...
class NormStateV400:
    """
    Normative state container per §3.1 of design freeze.
    ...
    @classmethod
    def from_dict(cls, d: Dict) -> "NormStateV400":
        return cls(
...
def create_initial_norm_state() -> NormStateV400:
    """
    Create the initial NormState per §3.2 of design freeze.
...
def expire_rules(state: NormStateV400, current_episode: int) -> NormStateV400:
    """
...
def serialize_state(state: NormStateV400) -> str:
    """Serialize state to canonical JSON string."""
    return canonicalize(state.to_dict())


def deserialize_state(json_str: str) -> NormStateV400:
    """Deserialize state from JSON string."""
    import json
    return NormStateV400.from_dict(json.loads(json_str))
