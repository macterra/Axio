"""
RSA-PoC v4.3 — TriDemandV430 Environment
Implements two-repair multi-sovereignty pressure model.

v4.3 additions over v4.2:
- regime ∈ {0, 1, 2} with event-gated regime-2 transition (E3)
- Contradiction B under regime=2: dual delivery obligations blocked by R7/R8
- progress_set for DELIVER_A/B returns only DEPOSIT at matching zone
- Epoch-chained normative continuity (epoch_0 → epoch_1 → epoch_2)
- Non-subsumption replay support
...
# ============================================================================
# Action Space (unchanged from v4.2)
# ============================================================================


class Action(IntEnum):
    """Action IDs per v4.2 (unchanged for v4.3)."""
    MOVE_N = 0   # A0
    MOVE_S = 1   # A1
    MOVE_E = 2   # A2
    MOVE_W = 3   # A3
...
# ============================================================================
# Grid Layout (unchanged from v4.2)
# ============================================================================
...
# ============================================================================
# State Space (extended for v4.3)
# ============================================================================


@dataclass
class TriDemandState430:
    """
    State representation for v4.3.

    v4.3 changes:
    - regime: 0, 1, or 2 (event-gated)
    - dual_delivery_mode: True when regime==2 (S_B predicate)
    - item_type: Type of item in inventory ('A', 'B', or None) for can_deliver predicates
    """
...
@dataclass
class Observation430:
    """Observation passed to deliberator/compiler (v4.3)."""
    agent_pos: Tuple[int, int]
    inventory: int
    ...
    @classmethod
    def from_state(cls, state: TriDemandState430) -> "Observation430":
        return cls(
...
# ============================================================================
# Obligation Target Interface (v4.3)
# ============================================================================
...
def target_satisfied(obs: Observation430, obligation_target: Dict[str, str]) -> bool:
    """
    Check if an obligation target is satisfied.

    Per §5.5: DEPOSIT_ZONE targets are satisfied when zone_X_satisfied == True.
    v4.3: Also handles DELIVER_A and DELIVER_B targets.
    """
...
def rank(obs: Observation430, obligation_target: Dict[str, str]) -> int:
    """
    ...
    # Under regime=1, if not stamped, add stamp path
...
def progress_set(obs: Observation430, obligation_target: Dict[str, str]) -> Set[str]:
    """
...
def simulate_step(obs: Observation430, action_id: str) -> Observation430:
    """
    ...
    if action == Action.MOVE_N:
        ...
        if pos == POSITIONS["SOURCE"] and inv < 3:
            new_inv = inv + 1
            # Collect alternates between A and B items
...
class TriDemandV430:
    """
    5×5 gridworld environment for v4.3.

    v4.3 additions over v4.2:
    - regime ∈ {0, 1, 2} with event-gated transition
    - Contradiction B under regime=2 (dual delivery blocked)
    - Epoch-chained normative continuity
    - Non-subsumption replay support
    """
    ...
    def __init__(
        ...
        # State
        self.state: Optional[TriDemandState430] = None
        self.episode_count = 0
        self.total_steps = 0

        # v4.3: Event-gated regime-2 transition
        self._repair_a_episode: Optional[int] = None  # Episode when Repair A was accepted
        self._regime_2_actual_start: Optional[int] = None  # Computed regime-2 start

        # v4.3: Epoch chain (multiple nonces for epoch_0, epoch_1, epoch_2)
        self._nonces: List[bytes] = []  # nonce_0, nonce_1, nonce_2
    ...
    def _compute_regime(self, episode: int) -> int:
        ...
        - regime 0: episode < regime_1_start
        - regime 1: regime_1_start <= episode < regime_2_actual_start
        - regime 2: episode >= regime_2_actual_start
        """
        ...
        # Repair A not yet accepted - stay in regime 1 if past regime_1_start
        return 1
    ...
    def record_repair_a_accepted(self, episode: int) -> None:
        ...
        self._regime_2_actual_start = max(
            ...
            episode + 1  # At least one episode under post-A law
        )
    ...
    def get_nonce(self, index: int) -> bytes:
        ...
        nonce_0: for epoch_0
        nonce_1: for epoch_1 (after Repair A)
        nonce_2: for epoch_2 (after Repair B)
        """
        while len(self._nonces) <= index:
            self._nonces.append(secrets.token_bytes(32))
        return self._nonces[index]

    def reset(self, episode: Optional[int] = None) -> Tuple[Observation430, Dict[str, Any]]:
        """
        Reset environment for new episode.

        v4.3: Applies event-gated regime transitions.
        """
        ...
        obs = Observation430.from_state(self.state)
        info = {
            "episode": self.episode_count,
            "rule_r1_active": rule_r1_active,
            "regime": regime,
    ...
    def step(self, action: Union[int, Action, str]) -> Tuple[Observation430, float, bool, bool, Dict[str, Any]]:
        """
    ...
    def target_satisfied(self, obs: Observation430, obligation_target: Dict[str, str]) -> bool:
        """Check if obligation target is satisfied."""
        return target_satisfied(obs, obligation_target)

    def rank(self, obs: Observation430, obligation_target: Dict[str, str]) -> int:
        """Compute rank for obligation target."""
        return rank(obs, obligation_target)

    def progress_set(self, obs: Observation430, obligation_target: Dict[str, str]) -> Set[str]:
        """Compute progress set for obligation target."""
        return progress_set(obs, obligation_target)

    # ========================================================================
    # Contradiction B Support (v4.3)
    # ========================================================================

    def get_active_delivery_obligations(self) -> List[Dict[str, str]]:
        """
        Get active DELIVER obligations under regime=2.

        Returns [DELIVER_A, DELIVER_B] when in regime 2.
        """
        if self.regime != 2:
            return []

        return [
            {"kind": "DELIVER", "target_id": "ZONE_A"},
            {"kind": "DELIVER", "target_id": "ZONE_B"},
        ]
    ...
    def get_observation(self) -> Observation430:
        """Get current observation without stepping."""
        if self.state is None:
            raise RuntimeError("Environment not reset")
        return Observation430.from_state(self.state)
    ...
    def render(self) -> str:
        ...
        lines.append(f"can_deliver_a: {self.state.can_deliver_a}, can_deliver_b: {self.state.can_deliver_b}")
