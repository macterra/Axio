#!/usr/bin/env python3
"""
RSA-PoC v4.3 — LLM Baseline Run (Single Seed)
...
Usage:
    python -m src.rsa_poc.v430.run_llm_baseline
    python -m src.rsa_poc.v430.run_llm_baseline --seed 123 --episodes 10
    python -m src.rsa_poc.v430.run_llm_baseline --preflight-only  # Validate only, no LLM
"""
...
def run_preflight_validation(
    ...
    3. Dual repair and persistence work
    ...
    if success_rate < 0.8:  # Allow some variance
        valid = False
        issues.append(f"Success rate {success_rate:.1%} < 80%")

    # v4.3: May have 0, 1, or 2 repairs depending on regimes encountered
    if harness.total_repairs_accepted > 2:
        valid = False
        issues.append(f"Repairs {harness.total_repairs_accepted} > 2 (R9 violation)")

    if verbose:
        print(f"  Success rate: {successes}/{max_episodes} = {success_rate:.1%}")
        print(f"  Repairs: {harness.total_repairs_accepted} (A: {harness.repair_a_accepted}, B: {harness.repair_b_accepted})")
        print(f"  Halts: {harness.total_halts}")
        print(f"  Epoch chain: {len(harness.epoch_chain)} epochs")
        if valid:
...
def run_with_llm(
    ...
    for ep in range(max_episodes):
        ...
        if ep_result["success"]:
            successes += 1
        if verbose:
            print(f"  Ep{ep}: steps={ep_result['steps']}, success={ep_result['success']}, regime={ep_result['regime_at_end']}")
    ...
    result = {
        ...
        "summary": {
            ...
            "repair_a_accepted": harness.repair_a_accepted,
            "repair_b_accepted": harness.repair_b_accepted,
            "total_successes": successes,
            "episodes_completed": max_episodes,
            "success_rate": successes / max_episodes
        },
        "epoch_chain": harness.epoch_chain,
        "gate_telemetry": {
        ...
        "telemetry": harness.telemetry.to_dict(),
    ...
    if verbose:
        ...
        print(f"    Repairs: A={harness.repair_a_accepted}, B={harness.repair_b_accepted}")
        print(f"    Epoch chain: {len(harness.epoch_chain)} epochs")
...
def main():
    parser = argparse.ArgumentParser(
        description="RSA-PoC v4.3 LLM Baseline Run"
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output file path (default: results/v430/v430_llm_baseline_<seed>_<timestamp>.json)"
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
