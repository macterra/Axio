"""
RSA-PoC v4.3 — Trace System with Stable TraceEntryID

v4.3 inherits all v4.2 trace functionality:
...
v4.3 additions:
- CONTRADICTION_A / CONTRADICTION_B distinction in data
- regime field in contradiction entries
...
class TraceEntry:
    ...
    v4.3 additions:
    - regime: Current regime when entry was created
    - contradiction_type: 'A' or 'B' for CONTRADICTION entries
    """
    trace_entry_id: str
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.contradiction_type is not None:
            d["contradiction_type"] = self.contradiction_type
        return d
...
def create_contradiction_entry(
    ...
    """
    Create a CONTRADICTION trace entry with all required fields (v4.3).

    This entry is cited by LAW_REPAIR actions (R7).

    Args:
        contradiction_type: 'A' (regime=1, stamp blocked) or 'B' (regime=2, deposit blocked)
    """
...
def create_contradiction_a_entry(
    run_seed: int,
    episode: int,
    step: int,
    blocking_rule_ids: List[str],
    ...
    """Create a Contradiction A entry (regime=1, stamp blocked by R6)."""
    return create_contradiction_entry(
        run_seed=run_seed,
        episode=episode,
        step=step,
        blocking_rule_ids=blocking_rule_ids,
...
def create_contradiction_b_entry(
    run_seed: int,
    episode: int,
    step: int,
    blocking_rule_ids: List[str],
    ...
    """Create a Contradiction B entry (regime=2, deposit blocked by R7/R8)."""
    return create_contradiction_entry(
        run_seed=run_seed,
        episode=episode,
        step=step,
...
class TraceLog:
    ...
    def get_contradictions(
        ...
        """Get all CONTRADICTION entries, optionally filtered by episode or type."""
        result = [
        ...
        if contradiction_type is not None:
            result = [e for e in result if e.contradiction_type == contradiction_type]
        return result
    ...
    def get_contradictions_a(self) -> List[TraceEntry]:
        """Get all Contradiction A entries."""
        return self.get_contradictions(contradiction_type="A")

    def get_contradictions_b(self) -> List[TraceEntry]:
        """Get all Contradiction B entries."""
        return self.get_contradictions(contradiction_type="B")
    ...
    def add_contradiction_a(
        ...
        """Add a Contradiction A entry."""
        return self.add_contradiction(
            episode=episode,
            step=step,
            blocking_rule_ids=blocking_rule_ids,
            active_obligation_target=active_obligation_target,
