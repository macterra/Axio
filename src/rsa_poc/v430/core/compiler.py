"""
RSA-PoC v4.3 — JCOMP-4.3 Compiler

v4.3 additions:
- POSITION_EQ: Check position == value
- CAN_DELIVER_A: Check can_deliver_a == True
- CAN_DELIVER_B: Check can_deliver_b == True
- Multi-obligation handling for Contradiction B
- Epoch chain awareness
"""

from __future__ import annotations

import hashlib
import inspect
import json
from dataclasses import dataclass, field
from enum import Enum
from typing import Any, Callable, Dict, List, Optional, Set, Tuple

from .dsl import (
    ActionClass,
    Condition,
    ConditionOp,
    EffectType,
    JustificationV430,
    NormativeRule,
    PatchOp,
    PatchOperation,
    RuleType,
)
from .norm_state import NormStateV430


# ============================================================================
# Compiler Hash (R8)
# ============================================================================


def compute_compiler_hash() -> str:
    """
    Compute hash of this compiler module for R8 validation.

    Returns SHA-256 of the module source code.
    """
    source = inspect.getsource(inspect.getmodule(compute_compiler_hash))
    return hashlib.sha256(source.encode('utf-8')).hexdigest()


# Module-level compiler hash (computed at import time)
JCOMP430_HASH = compute_compiler_hash()


# ============================================================================
# Action Class Mapping
# ============================================================================

ACTION_CLASS_TO_ACTION_IDS: Dict[str, Set[str]] = {
    "MOVE": {"A0", "A1", "A2", "A3"},
    "COLLECT": {"A4"},
    "DEPOSIT": {"A5"},
    "STAMP": {"A6"},
    "ANY": {"A0", "A1", "A2", "A3", "A4", "A5", "A6"},
    "WAIT": set(),
}


# ============================================================================
# Compilation Status
# ============================================================================


class CompilationStatus(str, Enum):
    """Compilation result status."""
    COMPILED = "COMPILED"
    PARSE_ERROR = "PARSE_ERROR"
    SCHEMA_ERROR = "SCHEMA_ERROR"
    REFERENCE_ERROR = "REFERENCE_ERROR"


# ============================================================================
# RuleEval (Executable Predicate Form)
# ============================================================================


@dataclass
class RuleEvalV430:
    """
    Compiled rule evaluator for v4.3.

    Includes repair_epoch for continuity tracking.
    """
    rule_id: str
    rule_type: str  # "PERMISSION", "PROHIBITION", "OBLIGATION"
    compiled_norm_hash: str
    condition_fn: Callable[[Any], bool]
    exception_fn: Optional[Callable[[Any], bool]] = None  # v4.3: Exception condition
    effect: Dict[str, Any] = field(default_factory=dict)
    priority: int = 0
    repair_epoch: Optional[str] = None

    def active(self, obs: Any, current_norm_hash: str) -> bool:
        """
        Check if rule is active for given observation.

        v4.3: Checks exception condition - rule is inactive if exception is True.
        """
        if current_norm_hash != self.compiled_norm_hash:
            return False

        # Check base condition
        if not self.condition_fn(obs):
            return False

        # Check exception - if True, rule is NOT active
        if self.exception_fn is not None and self.exception_fn(obs):
            return False

        return True


# ============================================================================
# Compilation Result
# ============================================================================


@dataclass
class CompilationResultV430:
    """Result of compiling a single justification."""
    action_id: str
    status: CompilationStatus
    rule_evals: Optional[List[RuleEvalV430]] = None
    error_message: Optional[str] = None
    compiler_hash: str = JCOMP430_HASH


@dataclass
class CompilationBatchV430:
    """Result of compiling multiple justifications."""
    results: List[CompilationResultV430]
    compiled_count: int = 0
    failed_count: int = 0
    compiler_hash: str = JCOMP430_HASH

    def __post_init__(self):
        self.compiled_count = sum(
            1 for r in self.results if r.status == CompilationStatus.COMPILED
        )
        self.failed_count = len(self.results) - self.compiled_count


# ============================================================================
# Condition Compilation (v4.3 extended)
# ============================================================================


def compile_condition(cond: Condition) -> Callable[[Any], bool]:
    """
    Compile a Condition into a callable predicate.

    v4.3 additions:
    - POSITION_EQ: Check position == value
    - CAN_DELIVER_A: Check can_deliver_a == True
    - CAN_DELIVER_B: Check can_deliver_b == True
    """
    op = cond.op
    args = cond.args

    if op == ConditionOp.TRUE:
        return lambda obs: True

    elif op == ConditionOp.FALSE:
        return lambda obs: False

    elif op == ConditionOp.EQ:
        field_name = args[0]
        value = args[1]
        return lambda obs, f=field_name, v=value: _get_obs_field(obs, f) == v

    elif op == ConditionOp.GT:
        field_name = args[0]
        value = args[1]
        return lambda obs, f=field_name, v=value: _get_obs_field(obs, f) > v

    elif op == ConditionOp.LT:
        field_name = args[0]
        value = args[1]
        return lambda obs, f=field_name, v=value: _get_obs_field(obs, f) < v

    elif op == ConditionOp.IN_STATE:
        state_id = args[0]
        return lambda obs, s=state_id: _check_in_state(obs, s)

    elif op == ConditionOp.HAS_RESOURCE:
        n = args[0]
        return lambda obs, required=n: _get_obs_field(obs, "inventory") >= required

    elif op == ConditionOp.AND:
        sub_fns = [compile_condition(a) if isinstance(a, Condition)
                   else compile_condition(Condition.from_dict(a)) for a in args]
        return lambda obs, fns=sub_fns: all(fn(obs) for fn in fns)

    elif op == ConditionOp.OR:
        sub_fns = [compile_condition(a) if isinstance(a, Condition)
                   else compile_condition(Condition.from_dict(a)) for a in args]
        return lambda obs, fns=sub_fns: any(fn(obs) for fn in fns)

    elif op == ConditionOp.NOT:
        sub_fn = compile_condition(args[0]) if isinstance(args[0], Condition) \
                 else compile_condition(Condition.from_dict(args[0]))
        return lambda obs, fn=sub_fn: not fn(obs)

    # v4.2 conditions (inherited)
    elif op == ConditionOp.REGIME_EQ:
        value = args[0]
        return lambda obs, v=value: _get_obs_field(obs, "regime") == v

    elif op == ConditionOp.STAMPED:
        return lambda obs: _get_obs_field(obs, "stamped") == True

    # v4.3 conditions
    elif op == ConditionOp.POSITION_EQ:
        value = args[0]
        return lambda obs, v=value: _get_obs_field(obs, "position") == v

    elif op == ConditionOp.CAN_DELIVER_A:
        return lambda obs: _get_obs_field(obs, "can_deliver_a") == True

    elif op == ConditionOp.CAN_DELIVER_B:
        return lambda obs: _get_obs_field(obs, "can_deliver_b") == True

    else:
        raise ValueError(f"Unknown condition operator: {op}")


def _get_obs_field(obs: Any, field: str) -> Any:
    """Get a field from observation (supports dict or dataclass)."""
    if isinstance(obs, dict):
        return obs.get(field)
    return getattr(obs, field, None)


def _check_in_state(obs: Any, state_id: str) -> bool:
    """Check if agent is in a named state/position."""
    from ..env.tri_demand import POSITIONS

    target_pos = POSITIONS.get(state_id)
    if target_pos is None:
        return False

    agent_pos = _get_obs_field(obs, "position")
    return agent_pos == target_pos


# ============================================================================
# JCOMP-4.3 Compiler
# ============================================================================


class JCOMP430:
    """
    JCOMP-4.3: Deterministic justification compiler.

    v4.3 additions:
    - Exception condition compilation
    - Multi-obligation support for Contradiction B
    - Epoch chain awareness
    """

    def __init__(self, norm_state: NormStateV430):
        self.norm_state = norm_state
        self.compiler_hash = JCOMP430_HASH

    def compile(self, justification_json: str) -> CompilationResultV430:
        """Compile a single justification from JSON string."""
        try:
            data = json.loads(justification_json)
        except json.JSONDecodeError as e:
            return CompilationResultV430(
                action_id="UNKNOWN",
                status=CompilationStatus.PARSE_ERROR,
                error_message=f"Invalid JSON: {e}"
            )

        try:
            justification = JustificationV430.from_dict(data)
        except (KeyError, ValueError, TypeError) as e:
            action_id = data.get("action_id", "UNKNOWN")
            return CompilationResultV430(
                action_id=action_id,
                status=CompilationStatus.SCHEMA_ERROR,
                error_message=f"Schema validation failed: {e}"
            )

        return self.compile_justification(justification)

    def compile_justification(self, justification: JustificationV430) -> CompilationResultV430:
        """Compile a validated JustificationV430 object."""
        action_id = justification.action_id

        # Reference resolve
        for rule_ref in justification.rule_refs:
            if not self.norm_state.has_rule(rule_ref):
                return CompilationResultV430(
                    action_id=action_id,
                    status=CompilationStatus.REFERENCE_ERROR,
                    error_message=f"Rule reference not found: {rule_ref}"
                )

        # Emit RuleEval for each referenced rule
        rule_evals = []
        for rule_ref in justification.rule_refs:
            rule = self.norm_state.get_rule(rule_ref)
            if rule is None:
                return CompilationResultV430(
                    action_id=action_id,
                    status=CompilationStatus.REFERENCE_ERROR,
                    error_message=f"Rule not found: {rule_ref}"
                )

            condition_fn = compile_condition(rule.condition)

            # Compile exception condition if present
            exception_fn = None
            if rule.exception_condition is not None:
                exception_fn = compile_condition(rule.exception_condition)

            rule_eval = RuleEvalV430(
                rule_id=rule.id,
                rule_type=rule.type.value,
                compiled_norm_hash=self.norm_state.norm_hash,
                condition_fn=condition_fn,
                exception_fn=exception_fn,
                effect=rule.effect.to_dict(),
                priority=rule.priority,
                repair_epoch=rule.repair_epoch,
            )
            rule_evals.append(rule_eval)

        return CompilationResultV430(
            action_id=action_id,
            status=CompilationStatus.COMPILED,
            rule_evals=rule_evals
        )

    def compile_batch(self, justifications_json: List[str]) -> CompilationBatchV430:
        """Compile multiple justifications from JSON strings."""
        results = [self.compile(j) for j in justifications_json]
        return CompilationBatchV430(results=results)

    def compile_justifications(self, justifications: List[JustificationV430]) -> CompilationBatchV430:
        """Compile multiple validated justifications."""
        results = [self.compile_justification(j) for j in justifications]
        return CompilationBatchV430(results=results)

    def compile_all_rules(self) -> List[RuleEvalV430]:
        """
        Compile all rules in norm_state to RuleEval objects.

        Used for full law compilation (not tied to specific justifications).
        """
        rule_evals = []
        for rule in self.norm_state.rules:
            condition_fn = compile_condition(rule.condition)

            # Compile exception condition if present
            exception_fn = None
            if rule.exception_condition is not None:
                exception_fn = compile_condition(rule.exception_condition)

            rule_eval = RuleEvalV430(
                rule_id=rule.id,
                rule_type=rule.type.value,
                compiled_norm_hash=self.norm_state.norm_hash,
                condition_fn=condition_fn,
                exception_fn=exception_fn,
                effect=rule.effect.to_dict(),
                priority=rule.priority,
                repair_epoch=rule.repair_epoch,
            )
            rule_evals.append(rule_eval)
        return rule_evals


# ============================================================================
# Mask Stage (Obligation Gate) - v4.3
# ============================================================================


@dataclass
class MaskResultV430:
    """Result of the Mask stage with Obligation Gate for v4.3."""
    feasible: List[str]
    feasible_count: int
    binding_obligations: Optional[List[str]] = None  # v4.3: Can have multiple
    binding_obligation_targets: Optional[List[Dict[str, str]]] = None
    error: Optional[str] = None
    is_contradiction: bool = False
    contradiction_type: Optional[str] = None  # 'A' or 'B'
    blocking_rule_ids: Optional[List[str]] = None
    progress_actions: Optional[Set[str]] = None
    compiled_permitted_actions: Optional[Set[str]] = None


class EnvironmentInterfaceV430:
    """Abstract interface for environment obligation methods (v4.3)."""

    def target_satisfied(self, obs: Any, obligation_target: Dict[str, Any]) -> bool:
        raise NotImplementedError

    def rank(self, obs: Any, obligation_target: Dict[str, Any]) -> int:
        raise NotImplementedError

    def progress_set(self, obs: Any, obligation_target: Dict[str, Any]) -> Set[str]:
        raise NotImplementedError


def compute_feasible_430(
    compiled_rules: List[RuleEvalV430],
    norm_state: NormStateV430,
    obs: Any,
    env: EnvironmentInterfaceV430,
    current_norm_hash: str,
    regime: int,
) -> MaskResultV430:
    """
    Compute feasible actions with Obligation Gate per v4.3.

    v4.3: Handles multiple simultaneous obligations (Contradiction B).
    """
    # Step 1: Determine active rules
    active_perms: List[RuleEvalV430] = []
    active_prohibs: List[RuleEvalV430] = []
    active_obligs: List[RuleEvalV430] = []

    for r in compiled_rules:
        if not r.active(obs, current_norm_hash):
            continue
        if r.rule_type == "PERMISSION":
            active_perms.append(r)
        elif r.rule_type == "PROHIBITION":
            active_prohibs.append(r)
        elif r.rule_type == "OBLIGATION":
            active_obligs.append(r)

    # Step 2: Build permitted/prohibited sets
    permitted: Set[str] = set()
    prohibited: Set[str] = set()

    for r in active_perms:
        if r.effect.get("effect_type") == "ACTION_CLASS":
            action_class = r.effect.get("action_class", "")
            permitted |= ACTION_CLASS_TO_ACTION_IDS.get(action_class, set())

    for r in active_prohibs:
        if r.effect.get("effect_type") == "ACTION_CLASS":
            action_class = r.effect.get("action_class", "")
            prohibited |= ACTION_CLASS_TO_ACTION_IDS.get(action_class, set())

    compiled_permitted_actions = permitted - prohibited

    # Step 3: Handle obligations
    if not active_obligs:
        return MaskResultV430(
            feasible=list(compiled_permitted_actions),
            feasible_count=len(compiled_permitted_actions)
        )

    # Get all active obligation targets
    obligation_targets: List[Dict[str, str]] = []
    obligation_rule_ids: List[str] = []

    for r in active_obligs:
        eff = r.effect
        if eff.get("effect_type") != "OBLIGATION_TARGET":
            continue
        tgt = eff.get("obligation_target")
        if tgt is None:
            continue

        # Skip satisfied obligations
        if env.target_satisfied(obs, tgt):
            continue

        obligation_targets.append(tgt)
        obligation_rule_ids.append(r.rule_id)

    if not obligation_targets:
        # All obligations satisfied
        return MaskResultV430(
            feasible=list(compiled_permitted_actions),
            feasible_count=len(compiled_permitted_actions)
        )

    # Compute union of progress sets
    all_progress: Set[str] = set()
    for tgt in obligation_targets:
        ps = env.progress_set(obs, tgt)
        all_progress |= ps

    if not all_progress:
        return MaskResultV430(
            feasible=[],
            feasible_count=0,
            binding_obligations=obligation_rule_ids,
            binding_obligation_targets=obligation_targets,
        )

    feasible = list(all_progress & compiled_permitted_actions)

    # Detect contradiction
    if not feasible:
        # Find blocking prohibitions
        blocking_rule_ids = []
        for r in active_prohibs:
            if r.effect.get("effect_type") == "ACTION_CLASS":
                action_class = r.effect.get("action_class", "")
                prohibited_actions = ACTION_CLASS_TO_ACTION_IDS.get(action_class, set())
                if all_progress & prohibited_actions:
                    blocking_rule_ids.append(r.rule_id)

        # Determine contradiction type based on regime and blocking rules
        contradiction_type = _classify_contradiction(regime, blocking_rule_ids)

        return MaskResultV430(
            feasible=[],
            feasible_count=0,
            binding_obligations=obligation_rule_ids,
            binding_obligation_targets=obligation_targets,
            is_contradiction=True,
            contradiction_type=contradiction_type,
            blocking_rule_ids=blocking_rule_ids,
            progress_actions=all_progress,
            compiled_permitted_actions=compiled_permitted_actions,
        )

    return MaskResultV430(
        feasible=feasible,
        feasible_count=len(feasible),
        binding_obligations=obligation_rule_ids,
        binding_obligation_targets=obligation_targets,
    )


def _classify_contradiction(regime: int, blocking_rule_ids: List[str]) -> str:
    """
    Classify contradiction type based on regime and blocking rules.

    - Contradiction A: regime=1, blocked by R6 (STAMP prohibition)
    - Contradiction B: regime=2, blocked by R7/R8 (DEPOSIT prohibition)
    """
    if regime == 1:
        return 'A'
    elif regime == 2:
        # Check if blocking rules are R7/R8
        b_rules = {'R7', 'R8'}
        if b_rules.intersection(set(blocking_rule_ids)):
            return 'B'
    return 'A'  # Default to A for safety


# ============================================================================
# Exports
# ============================================================================

__all__ = [
    "JCOMP430",
    "JCOMP430_HASH",
    "compute_compiler_hash",
    "CompilationStatus",
    "RuleEvalV430",
    "CompilationResultV430",
    "CompilationBatchV430",
    "compile_condition",
    "MaskResultV430",
    "EnvironmentInterfaceV430",
    "compute_feasible_430",
    "ACTION_CLASS_TO_ACTION_IDS",
]
