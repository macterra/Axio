...
# Add V120/src to path for frozen imports
V120_SRC = Path(__file__).parent
sys.path.insert(0, str(V120_SRC))
...
from env_violation_wrapper import (
    CalibMazeV120, T_VIOLATION, VIOLATION_ACTION,
...
# =============================================================================
# EXTENDED STEP LOG (per preregistration §14.1)
# =============================================================================
@dataclass
class StepLogASI1:
    ...
    payload_hash: bytes
    state_hash: bytes
    authority_actor: str            # PREDECESSOR | SUCCESSOR | NONE
    phase: str                      # PRE_COMMIT | POST_COMMIT | HOLD
    violation_event_id: Optional[str]  # violation event ID or null
...
# =============================================================================
# EXTENDED RUN LOG (per preregistration §14.2)
# =============================================================================
@dataclass
class RunLogASI1:
...
# =============================================================================
# PROVENANCE VERIFICATION (per preregistration §6.3)
# =============================================================================
def compute_binding_root(
    predecessor_state: NormativeStateASI0,
    commit_nonce: int,
    protocol_tag: str = "ASI1V02"
) -> bytes:
    """Compute binding root per preregistration §6.3."""
...
def inject_provenance_failure(computed_root: bytes) -> bytes:
    """One-bit flip in LSB per preregistration §6.3."""
...
def derive_successor_seed(master_seed: int) -> int:
    ...
    canonical = f"{master_seed}|SUCCESSOR|{T_COMMIT}|ASI1V02"
...
def run_episode(
    ...
    """
    # Initialize environment (V120 wrapper)
    env = CalibMazeV120()
    env.reset()
    ...
    for step in range(1, K + 1):
        ...
        # =====================================================================
        # AUTHORITY TRANSITION AT t_commit (per preregistration §5.2)
        # =====================================================================
        if step == T_COMMIT:
        ...
        # =====================================================================
        # GET FEASIBLE ACTIONS (per preregistration §3.2)
        # =====================================================================
        feasible_actions = env.get_feasible_actions(step, current_phase)

        # =====================================================================
        # VALIDATE VIOLATION CHOICE CONSTRAINTS (per preregistration §3.2, §8)
        # =====================================================================
        ...
        # =====================================================================
        # PHASE-SPECIFIC ACTION SELECTION (per preregistration §7, §8)
        # =====================================================================
        violation_event_id = None

        if current_phase == Phase.PRE_COMMIT:
            # Predecessor emits STAY only (per §7.2)
            ...
            # STAY is inert system action, not agent behavior
            if "STAY" not in masked_actions:
                masked_actions = {"STAY"}
            selected_action = "STAY"

        else:  # POST_COMMIT
            # Successor selects deterministically
            # Agent should avoid violations (select compliant actions)
        ...
        # =====================================================================
        # LOG STEP (per preregistration §14.1)
        # =====================================================================
...
def run_asi1_experiment() -> Dict[str, Any]:
    """Run the full ASI-1 v0.2 experiment per preregistration."""
    results = {
        "experiment_id": "PHASE-VII-ASI1-AUTHORIZED-SUCCESSOR-INJECTION-2",
        "version": "0.2",
...
if __name__ == "__main__":
    print("=" * 60)
    print("ASI-1 v0.2 Experiment Execution")
    print("=" * 60)
    ...
    output_file = output_dir / f"asi1_v02_results_{timestamp}.json"
