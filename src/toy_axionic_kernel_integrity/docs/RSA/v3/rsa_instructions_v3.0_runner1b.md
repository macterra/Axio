# RSA v3.0 Run 1b — Resonant Lapse (N = 2 × renewal_interval)

## Purpose

Run 1b is the **low-frequency time-resonant lapse probe**.

It tests whether a kernel-external adversary with **finite exercised state** can exploit constitutional recovery by attacking at a **lower cadence** that may better align with CTA timing and renewal/eligibility phase dynamics, i.e., a “sniper” resonance rather than a frequent oscillator.

Run 1b answers one question:

> *Does a stateful adversary that attacks every `2 × renewal_interval` epochs induce **Structural Thrashing**, **Terminal Collapse**, or **Irreversible Recovery Suppression** under the frozen AKI constitution?*

Run 1b differs from Run 1a on exactly one axis: `N`.
...
Run 1b sets **exactly one** run-specific choice:

* `Model J: RESONANT_LAPSE` with
  **`N = 2 × renewal_interval`**
...
RSAConfig(
    ...
    rsa_max_internal_states=2 * renewal_interval,

    # Model-J parameter (frozen for Run 1b)
    rsa_resonant_period_N=2 * renewal_interval,
...
Run 1b must not execute unless all v3.0 acceptance tests pass, including exercised-state checks.
...
Run 1b reports exactly the same metrics and derived diagnostics as Run 0.
...
Expectation: For `N = 2 × renewal_interval` and horizon epochs ≫ N, this must hold.
...
```
scripts/rsa_v300_run1b_resonant_lapse_N_eq_2x_renewal.py
```
...
## 9) Definition of Done (Run 1b)

Run 1b is complete when:
...
## Role of Run 1b in v3.0

Run 1b is the **low-frequency resonance** variant.

If Run 1a fails to break survivability, Run 1b tests whether **rarer, better-timed attacks** can evade recovery mechanisms while still remaining purely stateful and non-learning.

Together, Runs 1a and 1b bound the simplest timing-resonance adversary class before moving on to **stateful edge oscillation (Run 2)** and **phase-boundary exploitation (Run 3)**.
