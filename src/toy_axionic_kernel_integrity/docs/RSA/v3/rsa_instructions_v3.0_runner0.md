...
Establish a **clean reference baseline** for all RSA v3.0 experiments under the **exact frozen AKI configuration, horizon, and seed set** used in Runs 1a/1b/2/3.
...
This run anchors **AA / AAA / RTD** (and renewal/eligibility diagnostics) so all subsequent v3.0 degradation claims are comparative, not absolute.
...
* RSA v3.0 policy interface (must remain inactive in Run 0)
...
### 1.1 AKI v0.8 config (baseline for v3.0)

Use the same AKI config that will be used verbatim in Runs 1a/1b/2/3.
...
### 1.2 RSA v3.0 policy config (two required conditions)
...
Run with RSA v3.0 layer active, but with the no-op model. This validates that the *presence* of the RSA layer does not perturb AKI when it emits no overrides.
...
> Note: This equivalence check is mandatory because v3.0 introduces state-machine machinery and strengthened determinism constraints that must not have side effects when inert.

---

### 1.3 Seeds (fixed)

Use the same seed battery as all other v3.0 runs:
...
Run 0 uses the **exact same metrics and classifier** as Runs 1a/1b/2/3.
...
### 3.2 v3.0 derived diagnostics (per seed)
...
* confirmation that:
  ...
  * no internal state machine was instantiated **or** it remained inert and unconsulted
...
```
scripts/rsa_v300_run0_baseline.py
```
...
If they do not, **stop immediately** and do not proceed to Runs 1a/1b/2/3.
...
## Role of Run 0 in v3.0

Run 0 is not a stress test.
It is not an experiment.

It is the **ground truth reference** that makes every RSA v3.0 claim legible and defensible.
