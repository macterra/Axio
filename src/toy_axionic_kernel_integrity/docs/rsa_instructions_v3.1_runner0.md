...
Establish a **clean reference baseline** for all RSA v3.1 experiments under the **exact frozen AKI configuration, horizon, and seed set** used in Runs 1/2/3.

Run 0 exists to answer one question only:

> *What does governance look like with **no learning adversarial policy layer** under the v3.1 experimental setup?*

This run anchors **AA / AAA / RTD** (and renewal/eligibility diagnostics) so all subsequent v3.1 degradation claims are comparative, not absolute.
...
* Horizon, renewal interval, and epoch definition (use frozen v3.1 baseline)
* Seed set (fixed)
* Metrics computation (`AA`, `AAA`, `RTD`) and failure classifier (frozen)
* RSA v3.1 policy interface (must remain inactive in Run 0)
...
### 1.1 AKI v0.8 config (baseline for v3.1)

Use the same AKI config that will be used verbatim in Runs 1/2/3.
...
### 1.2 RSA v3.1 policy config (two required conditions)

Run 0 must be executed under **two conditions**, using the same seeds and horizon.

#### Condition A — RSA disabled (true clean baseline)

Run with no RSA config (or with RSA wrapper absent), i.e. RSA must not intercept actions.
...
Run with RSA v3.1 layer active, but with the no-op model. This validates that the *presence* of the RSA layer does not perturb AKI when it emits no overrides.
...
RSAConfig(
    ...
    rsa_max_internal_states=None,   # unused in NONE; must not be consulted
    rsa_max_learning_states=None,   # unused in NONE; must not be consulted
)
```
...
> Note: This equivalence check is mandatory because v3.1 introduces learning-state machinery, RNG provenance constraints, and epoch-hygiene enforcement that must not have side effects when inert.

---

### 1.3 Seeds (fixed)

Use the same seed battery as all other v3.1 runs:
...
Run 0 uses the **exact same metrics and classifier** as Runs 1/2/3.
...
### 3.2 v3.1 derived diagnostics (per seed)
...
* confirmation that:
  ...
  * no internal FSM state was instantiated **or** it remained inert and unconsulted
  * no learning-state machinery was instantiated **or** it remained inert and unconsulted
  * `rsa_max_internal_states` was not referenced for gating (NONE-path must not consult capacity constraints)
  * `rsa_max_learning_states` was not referenced for gating (NONE-path must not consult capacity constraints)
  * no epoch-hygiene checks were evaluated in a way that could alter control flow (NONE-path must be behaviorally inert)
...
```
scripts/rsa_v310_run0_baseline.py
```
...
If they do not, **stop immediately** and do not proceed to Runs 1/2/3.
...
## Role of Run 0 in v3.1

Run 0 is not a stress test.
It is not an experiment.

It is the **ground truth reference** that makes every RSA v3.1 claim legible and defensible.
