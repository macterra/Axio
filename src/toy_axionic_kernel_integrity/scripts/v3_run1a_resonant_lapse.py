#!/usr/bin/env python3
"""
RSA v3.0 Run 1a: Resonant Lapse Trigger (N = renewal_interval)

Model J: Counter-based adversary that emits INVALID_COMMIT at regular intervals.
- State: counter modulo N (N=50 for Run 1a)
...
# Model J Run 1a parameters
RESONANT_PERIOD_N = RENEWAL_CHECK_INTERVAL  # N = 50


@dataclass
class TelemetryAccumulator:
    """Accumulates telemetry during a single run."""
    epochs_state_0: int = 0  # Epochs in state 0 (emit INVALID_COMMIT)
    epochs_state_other: int = 0  # Epochs in other states (emit COMPLIANT)
...
@dataclass
class Run1aMetrics:
    """Metrics extracted for Run 1a analysis with telemetry."""
    seed: int
    AA_ppm: int
    failure_class: str
    lapse_count: int
    ...
    # Telemetry (required for activation + exercised state proof)
    epochs_state_0: int
...
def run_with_telemetry(seed: int, config: ALSConfigV080, policy_config: RSAPolicyConfig) -> Tuple[Run1aMetrics, TelemetryAccumulator]:
...
def compute_activation_probabilities(all_metrics: List[Run1aMetrics]) -> Dict[str, float]:
...
def main():
    print("=" * 80)
    print("RSA v3.0 Run 1a: RESONANT_LAPSE (N=50)")
    print("=" * 80)
    print()
    ...
    for seed in SEEDS:
        ...
        print(f"AA={metrics.AA_ppm}ppm, lapses={metrics.lapse_count}, "
              ...
              f"exercised_ok={metrics.exercised_state_dynamic_ok}")
    ...
    # Verify exercised state (dynamic)
    print()
    print("=" * 80)
    ...
    if activation_ok and all_exercised and terminal_failures == 0:
        print("\n✅ RUN 1a COMPLETE - Model J (N=50) produces no terminal failures")
        return 0
    else:
        print("\n❌ RUN 1a ISSUES DETECTED")
        return 1
