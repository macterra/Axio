#!/usr/bin/env python3
"""
RSA v0.1.2 Run 0: Governance DoS Sweep (Spec-Pure).

CRITICAL CHANGES from v0.1.1:
1. Uses ALSHarnessV080_SV (separate test harness) instead of modifying ALSConfigV080
2. ALSConfigV080 remains frozen per AKI v0.8 spec
3. Includes synthetic verifier calibration statistics
4. Reports per-seed AND aggregate metrics clearly

This preserves the "AKI v0.8 is frozen; RSA is additive; SV is an experimental
environment wrapper" principle.
...
# Import SV harness directly to avoid circular import issues
...
class SeedResult:
    ...
    # Synthetic verifier calibration
...
def run_dos_sweep():
    """Run the DoS sweep experiment with spec-pure methodology."""
    print("=" * 78)
    print("RSA v0.1.2 Run 0: Governance DoS Sweep (SPEC-PURE)")
    print(f"Generated: {datetime.now().isoformat()}")
    print("=" * 78)
    print()

    # AKI v0.8 configuration (FROZEN - no synthetic verifier fields)
    ...
    # Synthetic Verifier configuration (SEPARATE from AKI)
    sv_config = SVConfig(
        per_ci_pass_rate_ppm=800_000,  # 80% per-Ci => ~51% SEM_PASS
    )
    ...
    print(f"  Synthetic Verifier (TEST HARNESS):")
    print(f"    per_ci_pass_rate_ppm: {sv_config.per_ci_pass_rate_ppm}")
    print(f"    expected SEM_PASS rate: {sv_config.expected_sem_pass_rate():.3f}")
    print(f"  Seeds: {seeds}")
    ...
    for p_flip_ppm in p_flip_values:
        ...
        for seed in seeds:
            ...
            # Use ALSHarnessV080_SV (test harness) instead of modifying ALSConfigV080
            h = ALSHarnessV080_SV(
            ...
            if result.rsa:
                ...
                seed_result.epochs_evaluated = (
                    result.total_cycles // config.renewal_check_interval
                )
                seed_result.authority_epochs_total = (
                    seed_result.epochs_evaluated - seed_result.lapse_epochs_total
                )
        ...
        # Compute aggregates
        n = len(seeds)
        sweep_result.sum_targets = sum(
            sr.total_targets for sr in sweep_result.seed_results
        )
        sweep_result.sum_flips = sum(sr.total_flips for sr in sweep_result.seed_results)
        sweep_result.sum_pivotal_flips = sum(
            sr.pivotal_flips for sr in sweep_result.seed_results
        )
        ...
        sweep_result.mean_lapse_event_count = (
            sum(sr.lapse_event_count for sr in sweep_result.seed_results) / n
        )
        sweep_result.mean_lapse_epochs_total = (
            sum(sr.lapse_epochs_total for sr in sweep_result.seed_results) / n
        )
        sweep_result.mean_authority_uptime = (
            sum(sr.authority_uptime_fraction for sr in sweep_result.seed_results) / n
        )
        sweep_result.mean_recovery_yield = (
            sum(sr.recovery_yield for sr in sweep_result.seed_results) / n
        )

        # SV calibration aggregates
        sweep_result.mean_sv_c0_rate = (
            sum(sr.sv_c0_rate for sr in sweep_result.seed_results) / n
        )
        sweep_result.mean_sv_c1_rate = (
            sum(sr.sv_c1_rate for sr in sweep_result.seed_results) / n
        )
        sweep_result.mean_sv_c2_rate = (
            sum(sr.sv_c2_rate for sr in sweep_result.seed_results) / n
        )
        sweep_result.mean_sv_sem_pass_rate = (
            sum(sr.sv_sem_pass_rate for sr in sweep_result.seed_results) / n
        )
    ...
    print(
        f"{'p_flip':>10} {'p_flip':>8} {'Lapses':>8} {'Lapse':>10} {'Uptime':>8} "
        f"{'Flips':>8} {'Pivotal':>8} {'ROBUST':>7} {'DEG':>5} {'DOS':>5}"
    )
    print(
        f"{'(PPM)':>10} {'(%)':>8} {'(count)':>8} {'(epochs)':>10} {'':>8} "
    ...
    for r in results:
        print(
            f"{r.p_flip_ppm:>10} {r.p_flip_pct:>8.2f} "
    ...
    print("\n" + "=" * 78)
    print("SYNTHETIC VERIFIER CALIBRATION")
    print("=" * 78)
    print()
    print(
        f"Expected per-Ci rate: {sv_config.per_ci_pass_rate_ppm / 1_000_000:.3f} "
        f"(= {sv_config.per_ci_pass_rate_ppm} PPM)"
    )
    print(f"Expected SEM_PASS rate: {sv_config.expected_sem_pass_rate():.3f}")
    print()
    print(
        f"{'p_flip':>10} {'C0 obs':>10} {'C1 obs':>10} {'C2 obs':>10} {'SEM_PASS obs':>12}"
    )
    print("-" * 55)

    for r in results:
        print(
            f"{r.p_flip_ppm:>10} {r.mean_sv_c0_rate:>10.3f} "
            f"{r.mean_sv_c1_rate:>10.3f} {r.mean_sv_c2_rate:>10.3f} "
            f"{r.mean_sv_sem_pass_rate:>12.3f}"
        )
