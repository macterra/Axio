#!/usr/bin/env python3
"""
RSA v3.0 Run 1b — Resonant Lapse (N = 2 × renewal_interval = 100)

Canonical script per rsa_instructions_v3.0_runner1b.md

Tests whether a stateful adversary that attacks every 2×renewal_interval epochs
induces Structural Thrashing, Terminal Collapse, or Irreversible Recovery
Suppression under the frozen AKI constitution.

Model J: RESONANT_LAPSE with N=100
- State: counter modulo N
- Policy: S_t == 0 → INVALID_COMMIT; else → COMPLIANT
- Attack frequency: once per 100 epochs (once per 2 renewal checks)
"""
import sys
import os
...
# Run 1b specific: N = 2 × renewal_interval
RESONANT_PERIOD_N = 100  # = 2 × RENEWAL_CHECK_INTERVAL
...
@dataclass
class RunMetrics:
    """Metrics for Run 1b including v3.0 telemetry."""
    seed: int
...
def run_with_model_j(seed: int, config: ALSConfigV080, n: int) -> Tuple[Any, Dict[str, Any]]:
    ...
    # Simulate adversary to collect telemetry
...
def main() -> int:
    print("=" * 80)
    print("RSA v3.0 RUN 1b: RESONANT LAPSE (N = 100)")
    print("=" * 80)
    print(f"Protocol: RSA v3.0 (RSA-SA-0)")
    print(f"Model: J (RESONANT_LAPSE)")
    print(f"N = {RESONANT_PERIOD_N} (= 2 × renewal_check_interval)")
    print(f"Seeds: {SEEDS}")
    ...
    # ========================================================================
    print("=" * 80)
    print("RUN 1b: MODEL J (RESONANT_LAPSE, N=100)")
    print("=" * 80)
    ...
    # Summary
    # ========================================================================
    print_summary("Run 1b (Model J, N=100)", results)

    # ========================================================================
    # RTD
    # ========================================================================
    agg_rtd, all_dur = compute_aggregate_rtd(results)
    print_aggregate_rtd("Run 1b", agg_rtd, all_dur)
    ...
    # ========================================================================
    # Comparison with Baseline and Run 1a
    # ========================================================================
    print("\n" + "=" * 80)
    print("COMPARISON WITH RUN 0 BASELINE AND RUN 1a")
    print("=" * 80)
    ...
    run1a_lapses_mean = 1512.10

    run1b_aa_mean = statistics.mean([results[s].AA_ppm for s in SEEDS])
    run1b_lapses_mean = statistics.mean([results[s].lapse_count for s in SEEDS])

    print(f"  Run 0 (Baseline) AA:  {baseline_aa_mean} ppm")
    print(f"  Run 1a (N=50) AA:     {run1a_aa_mean:.1f} ppm (Δ = {run1a_aa_mean - baseline_aa_mean:+.1f})")
    print(f"  Run 1b (N=100) AA:    {run1b_aa_mean:.1f} ppm (Δ = {run1b_aa_mean - baseline_aa_mean:+.1f})")
    print()
    print(f"  Run 0 (Baseline) lapses:  {baseline_lapses_mean:.2f}")
    print(f"  Run 1a (N=50) lapses:     {run1a_lapses_mean:.2f}")
    print(f"  Run 1b (N=100) lapses:    {run1b_lapses_mean:.2f}")
    ...
    if terminal_failures > 0:
        print(f"RUN 1b RESULT: ⚠ {terminal_failures} TERMINAL FAILURE(S) DETECTED")
        print("Model J (N=100) induced constitutional failure.")
    else:
        print("RUN 1b RESULT: ✓ NO TERMINAL FAILURES")
        print("Model J (N=100) did not defeat constitutional recovery.")
